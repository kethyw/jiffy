...
const UserHint = ({ loading, hintText }) => (
  <div className="user-hint">
    ...
    either our spinner or hintText based on that, using a ernary operato{if/else) */}
...
class App extends Component {
  ...
  searchGiphy = async searchTerm => {
    this.setState({
      //here we set out loading state to be true
    ...
    try {
      ...
      const response = await fetch(
        `https://api.giphy.com/v1/gifs/search?api_key=Me2qGSAS521uVxeHXzNLcG8xnwE9Et6l&q=${
        searchTerm
      ...
      // here we check if they array of results is empty
  ...
  handleChange = event => {
    ...
    // on the input as the value, we have create what is called
    // a controlled input
    this.setState((prevState, props) => ({
  ...
  handleKeyPress = event => {
    ...
    if (value.length > 2 && event.key === 'Enter') {
      // here we call our searchGiphy using the search term
